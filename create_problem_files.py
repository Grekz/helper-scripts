...
def get_name(problem_name):
    ...
    problem_name_file = re.sub(
        r"[\s-]+", "", problem_name_file, flags=re.UNICODE)
    problem_file = problem_number_file + '_' + problem_name_file
    return problem_file

lang_dict_excludes = { 'basic' : ["golang", "scala", "kotlin"], 'jpjg' : ["ruby", "scala", "kotlin"] }
is_mac = os.name == 'posix'
ignored_langs = lang_dict_excludes[sys.argv[1]] if len(sys.argv) == 2 else (sys.argv[1:] or [])
print(ignored_langs)
problem_level = input(
    "input your problem type vals:( e=easy, m=medium, h=hard ) [default=e] :\n") or 'e'
...
languages = [
    ...
    ("kotlin", 'src' + slash, "kt", kotlin_template),
]
for lang in languages:
    (lang_name, lang_path, file_type, file_template) = lang
    if not lang_name in ignored_langs:
        formatted_path = template_path.format(
            problem_level, lang_name, lang_path, file_name, file_type)
